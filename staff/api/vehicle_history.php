...
if($regi){
	...
	$select_vehicle = $con->query("SELECT vb.*, sdin.staff_name as in_staff_name, v.parking_name, CONCAT_WS(' ', v.address,v.city,v.state) as parking_address FROM `vehicle_booking` as vb LEFT JOIN pa_users as v ON vb.vendor_id = v.id LEFT JOIN staff_details as sdin ON vb.staff_in = sdin.staff_id Where vb.vendor_id = ".$vendor_id." AND vb.vehicle_status = 'In' AND (FROM_UNIXTIME(vb.vehicle_in_date_time, '%Y-%m-%d') >= '".$getstart."' AND FROM_UNIXTIME(vb.vehicle_in_date_time, '%Y-%m-%d') <= '".$getend."') ORDER BY vb.id DESC");
