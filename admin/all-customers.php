  ...
  <section id="main-content">
    <section class="wrapper"> 
      ...
      <div class="row">
        <div class="col-lg-12">
          <section class="panel">
            <header class="panel-heading"> All Customers  <span style="float: right;"><!-- <a class="btn-info btn" href="add-user.php">Add New</a> --></span></header>
...
<option value="">Filter By Domain</option>
 <?php $select_domain = $con->query("SELECT substring_index(user_email, '@', -1) domain, COUNT(*) email_count FROM pa_users WHERE user_role = 'customer' GROUP BY substring_index(user_email, '@', -1) ORDER BY email_count DESC, domain"); 
...
?>
		...
		<div class="table-responsive">
          ...
          $select_query = $con->query("SELECT * FROM `pa_users` WHERE user_role = 'customer' ORDER BY id DESC");
                      ...
                      <td><?php echo $row['wallet_amount']; ?></td>
                       <td><?php 
                        if($row['user_status']==1){ ?>
                           <a class="" title="Deactivate" href="user-status.php?user_id=<?php echo $row['id'];?>&status=0&return=all-customers.php"><i class="icon-remove" style="color: red;"></i> </a>
                        <?php } else { ?>
                           <a class="" title="Activate" href="user-status.php?user_id=<?php echo $row['id'];?>&status=1&return=all-customers.php"><i class="icon-check" style="color: green;"></i> </a>
                        <?php } ?> &nbsp;&nbsp;</td>
                         <td><?php echo $row['register_date']; ?></td>
                      <td class="center" id="user-action-<?php echo $row['id'];?>"> 

                        <a class="" title="Customer Vehicle" href="customer-vehicle.php?user_id=<?php echo $row['id'];?>" id="<?php echo $row['id'];?>"><i class="material-icons" style="font-size: 15px;">local_car_wash</i> </a>&nbsp;&nbsp;

                         <a class="" title="View Wallet History" href="wallet-history.php?user_id=<?php echo $row['id'];?>" id="<?php echo $row['id'];?>"><i class="fa fa-google-wallet"></i></a>&nbsp;&nbsp;
