...
//$regi =  json_decode('{"vendor_id":"150","vehicle_type":"2W","vehicle_number":"RJ14DC5678","parking_status":"Out"}');
...
if($regi){
	...
	if($parking_status=='In'){
		...
		$return = false;
		$select_vehicle = $con->query("SELECT * FROM `vehicle_booking` Where vendor_id=".$vendor_id." AND vehicle_number ='".$vehicle_number."' AND vehicle_type  ='".$vehicle_type."' AND vehicle_status = 'In' limit 1");
		...
		if($numrows_vehicle==1) {
			...
			if($fullDays < 1){
				...
				//echo "SELECT * FROM `fare_info` where veh_type = '".$vehicle_type."' AND user_id = ".$vendor_id." AND hr_status !='max'"; 
				//$newHrs = 3;
