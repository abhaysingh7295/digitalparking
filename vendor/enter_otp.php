...
if (isset($_POST['submit'])) {
    ...
    $password = mysqli_real_escape_string($con, $_POST['check_otp']);
    $select_query = $con->query("SET NAMES utf8");

    $select_query = $con->query("SELECT * FROM `pa_users` where otp='" . $password . "' AND id='" . $_SESSION["current_user_ID"] . "' AND   user_role = 'vandor'");
    if ($row = $select_query->fetch_assoc()) {
        if ($row['user_status'] == 1) {
            ...
            $con->query("update `pa_users` SET otp = '' where id = '" . $row['id'] . "'");
