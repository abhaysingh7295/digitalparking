...
/*if($_GET['submit']){
  ...
  if($id){
    $vehicle_book_query = $con->query("SELECT ph.*, vb.vehicle_number,vb.vehicle_type,staff.staff_name FROM `payment_history` as ph JOIN vehicle_booking as vb ON ph.booking_id = vb.id LEFT JOIN staff_details as staff ON ph.staff_id = staff.staff_id WHERE vb.vendor_id = ".$user_id." AND ph.booking_id = '".$id."' ORDER BY ph.id DESC");
  } else {
     $vehicle_book_query = $con->query("SELECT ph.*, vb.vehicle_number,vb.vehicle_type,staff.staff_name FROM `payment_history` as ph JOIN vehicle_booking as vb ON ph.booking_id = vb.id LEFT JOIN staff_details as staff ON ph.staff_id = staff.staff_id WHERE vb.vendor_id = ".$user_id." ORDER BY ph.id DESC");
  }
}*/
...
if($_GET['submit']){
  $getstart = $_GET['start_date'];
  $getend = $_GET['end_date'];
  $vehicle_book_query = $con->query("SELECT ph.*, vb.vehicle_number,vb.vehicle_type,staff.staff_name FROM `payment_history` as ph JOIN vehicle_booking as vb ON ph.booking_id = vb.id LEFT JOIN staff_details as staff ON ph.staff_id = staff.staff_id WHERE (FROM_UNIXTIME(ph.payment_date_time, '%Y-%m-%d') >= '".$getstart."' AND FROM_UNIXTIME(ph.payment_date_time, '%Y-%m-%d') <= '".$getend."') AND ph.staff_id = ".$id." ORDER BY ph.id DESC");
} else {
        ...
        <div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="row align-items-center">
                    <div class="col-sm-6">
                        <h4 class="page-title">Staff Payment Recevied</h4>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-right">
                            <li class="breadcrumb-item"><a href="javascript:void(0);"><?php echo $site_settings_row['site_name']; ?></a></li>
                            <li class="breadcrumb-item active">Staff Payment Recevied</li>
                        </ol>
                    </div>
                </div>
                <!-- end row -->
