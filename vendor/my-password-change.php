...
$confirm_password=$_POST['confirm_password'];
    $email_check = $con->query("select * from pa_users where user_pass='".$old_password."' AND id='".$_SESSION["current_user_ID"]."'");
    if($email_check->num_rows==1){
        if($new_password!=$confirm_password){
            echo "<script> alert('Password Not Match')</script>";
        } else {
            $con->query("update pa_users set user_pass='".$new_password."' where id='".$_SESSION["current_user_ID"]."'");
            echo "<script> alert('Your new password has been change.')</script>";
        }
    } else {
        echo "<script> alert('Your Old password has been Wrong.')</script>";
    }
}
?>
