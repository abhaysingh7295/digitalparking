...
$vendor_details =  $con->query("SELECT CONCAT_WS(' ', u.first_name,u.last_name) as vendor_name, u.user_email, u.mobile_number as vendor_mobile, u.address, u.city, u.state from pa_users as u where u.id = '".$user_id."'");
 ...
 //   echo "IN"; exit;
  ...
  //$vehicle_book_query = $con->query("SELECT b.*, sdin.staff_name as in_staff_name, sdout.staff_name as out_staff_name, u.first_name FROM `vehicle_booking` as b LEFT JOIN pa_users as u ON b.customer_id = u.id LEFT JOIN staff_details as sdin ON b.staff_in = sdin.staff_id LEFT JOIN staff_details as sdout ON b.staff_out = sdout.staff_id where FROM_UNIXTIME(`vehicle_in_date_time`, '%Y-%m-%d') = (DATE_FORMAT(NOW(),'%Y-%m-%d')) AND b.vendor_id = '".$user_id."' AND (FROM_UNIXTIME(b.vehicle_in_date_time, '%Y-%m-%d') >= '".$getstart."' AND FROM_UNIXTIME(b.vehicle_in_date_time, '%Y-%m-%d') <= '".$getend."' OR FROM_UNIXTIME(b.vehicle_out_date_time, '%Y-%m-%d') >= '".$getstart."' AND FROM_UNIXTIME(b.vehicle_out_date_time, '%Y-%m-%d') <= '".$getend."') ORDER BY b.id DESC");
   ...
   //echo "SELECT b.*, sdin.staff_name as in_staff_name, sdout.staff_name as out_staff_name, u.first_name FROM `vehicle_booking` as b LEFT JOIN pa_users as u ON b.customer_id = u.id LEFT JOIN staff_details as sdin ON b.staff_in = sdin.staff_id LEFT JOIN staff_details as sdout ON b.staff_out = sdout.staff_id where FROM_UNIXTIME(`vehicle_in_date_time`, '%Y-%m-%d') = (DATE_FORMAT(NOW(),'%Y-%m-%d')) AND b.vendor_id = '".$user_id."' ORDER BY b.id DESC";
 
 $vendor_details =  $con->query("SELECT CONCAT_WS(' ', u.first_name,u.last_name) as vendor_name, u.user_email, u.mobile_number as vendor_mobile, u.address, u.city, u.state from pa_users as u where u.id = '".$user_id."'");
// echo "SELECT CONCAT_WS(' ', u.first_name,u.last_name) as vendor_name, u.user_email, u.mobile_number as vendor_mobile, u.address, u.city, u.state from pa_users as u where u.id = '".$user_id."'";
 //exit;
                            ...
                            <div class="form-group" style="width:500px !important;">
                              <div>
                                <div class="input-daterange input-group" id="vdate-range">
                                  <input type="text" class="form-control start_date" name="start_date" placeholder="Start Date" value="<?php echo $getstart; ?>" />
                                  <input type="text" class="form-control end_date" name="end_date" placeholder="End Date" value="<?php echo $getend; ?>"/>
                           ...
                           <div class="col-6 vendor_det">
                            ...
                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                  ...
                                  ?></td>
                                  <td><?php if($row['in_staff_name']){ echo '<strong>In : </strong>'.$row['in_staff_name'].'<br/>'; } if($row['out_staff_name']) { echo '<strong>Out : </strong>'.$row['out_staff_name']; } ?></td>
