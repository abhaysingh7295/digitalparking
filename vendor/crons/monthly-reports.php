...
$activate_subscriptions_plans = $con->query("select vs.vendor_id, v.user_email, CONCAT_WS(' ', v.first_name,v.last_name) as vendor_name, v.parking_name, CONCAT_WS(' ', v.address,v.city,v.state) as vendor_address from `vendor_subscriptions` as vs JOIN pa_users as v on vs.vendor_id = v.id where vs.status = 1 AND vs.monthly_report = 1 AND v.user_role = 'vandor'");
...
while($active_plans_row=$activate_subscriptions_plans->fetch_assoc())
{
		    		...
		    		<td>
					   ...
					   <tbody>
					      <tr style="border-collapse:collapse">
					         ...
					         <td style="border-collapse:collapse;border:1px solid lightgrey;padding:3px;width:161.51px;font-weight:bold" colspan="6">The Digital Parking Monthly Report<br></td>
		        ...
		        <tr>
		            <td>
	                    ...
	                    <?php if($VehicleArray) { ?>
		                    <tr>
		                        <td>
		                          <strong>Your today Parking Report is ready please see in blow :</strong>  <br/><br/>	
		                        </td>
		                    </tr>
	...
	<?php $message = ob_get_contents();
		...
		$to = $vendor_email;
		$subject = date('Y-m-d').' Parking Report';
		#SendEmailNotification($to,$subject,$message);
	}
}

// for monthly user report
...
if ($query->num_rows>0) {
	while ($result=$query->fetch_assoc()) {
        ...
        $a = "SELECT  count(ph.staff_id) as total_vehicles, ph.staff_id, sd.staff_name, vb.parking_type, sum(ph.booking_amount) as total_booking_amount,ph.payment_type,vb.vehicle_type FROM `vehicle_booking` as vb LEFT JOIN (select booking_id, staff_id, sum(amount) as booking_amount,payment_type from payment_history WHERE FROM_UNIXTIME(payment_date_time, '%Y-%m-%d') >= '" . $start_date . "' AND FROM_UNIXTIME(payment_date_time, '%Y-%m-%d') <= '".$end_date."' GROUP BY booking_id, staff_id,payment_type) as ph ON ph.booking_id = vb.id AND (ph.staff_id = vb.staff_in OR ph.staff_id = vb.staff_out) LEFT JOIN staff_details as sd ON sd.staff_id = ph.staff_id Where vb.vendor_id=" . $vendor_id . " AND (FROM_UNIXTIME(vb.vehicle_in_date_time, '%Y-%m-%d') >= '" . $start_date . "' AND FROM_UNIXTIME(vb.vehicle_in_date_time, '%Y-%m-%d') <= '".$end_date."' OR FROM_UNIXTIME(vb.vehicle_out_date_time, '%Y-%m-%d') >= '" . $start_date . "' AND FROM_UNIXTIME(vb.vehicle_out_date_time, '%Y-%m-%d') <= '".$end_date."') AND ph.staff_id IS NOT NULL AND ph.staff_id != 0 GROUP BY ph.staff_id, vb.parking_type ORDER BY ph.staff_id";
        ...
        $numrows_vehicle = $select_vehicle->num_rows;
            ...
            if ($numrows_vehicle > 0 || $select_staff_vehicle->num_rows > 0){
            	 ...
            	 $start = date("d/m/Y h:i A", strtotime($start_date));
            	 $end = date("d/m/Y h:i A", strtotime($end_date));
				...
				$link=SITE_URL.'vendor/allovervehiclereport.php?vendor_id='.$result['vendor_id'];
				
				$datas['parameters']=array(array('name'=>'parking_name','value'=>$name),array('name'=>'address','value'=>$address),array('name'=>'date','value'=>$start),array('name'=>'date','value'=>$end),array('name'=>'invoice_url','value'=>$link));
